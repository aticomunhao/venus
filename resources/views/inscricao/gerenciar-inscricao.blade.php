...
@section('content')
    <div class="container-fluid";>
            ...
            <form action="{{ route('index.insc') }}" class="form-horizontal mt-4" method="GET">
                <div class="row justify-content-center" style="text-align: midlle;">
                    ...
                    <div class="col-md-2 col-6 mb-2">Atividade
                        <select class="form-select select2" id="tipo_tratamento" name="tipo_tratamento">
                            ...
                            @foreach ($tipo_tratamento as $tipot)
                                <option value="{{ $tipot->idt }}"
                                    {{ request('tipo_tratamento') == $tipot->idt ? 'selected' : '' }}>
                    ...
                    <div class="col-md-1 col-6 mb-2">Semestre
                        <select class="form-select select2" id="semestre" name="semestre">
                            ...
                            @foreach ($tipo_semestre as $tipos)
                                <option value="{{ $tipos->ids }}"
                                    {{ request('semestre') == $tipos->ids ? 'selected' : '' }}>
                                    {{ $tipos->sigla }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-1 col-6 d-flex justify-content-center align-items-center">
                        <input class="btn btn-light btn-sm w-100" style="box-shadow: 1px 2px 5px #000000; margin:5px;"
                    ...
                    <div class="col-lg-1 col-6 d-flex justify-content-center align-items-center">
                        <button type="button" class="btn btn-info btn-sm w-100"
                            style="box-shadow: 1px 2px 5px #000000; margin:5px;" data-bs-toggle="modal"
                            data-bs-target="#maisfiltros" style="margin:5px;">
                            Mais filtros
                        </button>
                    </div>
                    <div class="col-lg-1 col-6 d-flex justify-content-center align-items-center">
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="maisfiltros" tabindex="-1" aria-labelledby="filtrosPopupLabel"
                        aria-hidden="true">
                                ...
                                <div class="modal-body">
                                    <div class="row px-3" style="margin-left: 10px; margin-right:10px;">
                                        ...
                                        <div class="col-12 mt3">Setor:
                                            <select class="form-select select2" id="" name="setor">
                                        ...
                                        <div class="col-12 mt-3">Modalidade:
                                            <select class="form-select" name="modalidade">
                                                ...
                                                @foreach ($tmodalidade as $modal)
                                                    <option value="{{ $modal->idm }}"
                                                        {{ $modal->idm == request('modalidade') ? 'selected' : '' }}>
                                                        {{ $modal->nomem }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-12 mt-3">Status:
                                            <select class="form-select" name="status">
                                                ...
                                                @foreach ($situacao as $situ)
                                                    <option value="{{ $situ->ids }}"
                                                        {{ $situ->ids == request('status') ? 'selected' : '' }}>
                                                        {{ $situ->descs }}</option>
                                                @endforeach
                                            </select>
                                        </div>
        ...
        <hr>
        <div class="row">
            Quantidade de inscrições: {{ $contar }}
        </div>
        <div class="row">
            <table class="table table-sm table-striped table-bordered border-secondary table-hover align-middle">
                ...
                <tbody style="font-size: 14px; color:#000000; text-align: center;">
                    @foreach ($inscricao as $insc)
                        <tr>
                            <td class="d-none d-lg-table-cell">{{ $insc->idi }}</td>
                            <td>{{ $insc->nome_completo }}</td>
                            <td>{{ $insc->stsigla }}</td>
                            <td class="d-none d-lg-table-cell">{{ $insc->nmodal }}</td>
                            <td>{{ $insc->nomed }}</td>
                            <td>{{ $insc->numero }}</td>
                            <td class="d-none d-lg-table-cell">{{ $insc->trsigla }}-{{ $insc->trnome }}</td>
                            <td class="d-none d-lg-table-cell">{{ $insc->sesigla }}</td>
                            <td class="d-none d-lg-table-cell">{{ date('H:i', strtotime($insc->h_inicio)) }}</td>
                            <td class="d-none d-lg-table-cell">{{ date('d-m-Y', strtotime($insc->data_inicio)) }}</td>
                            <td>{{ $insc->tipo }}</td>
                            <td><i class="bi bi-info-circle-fill"
                                    style="color: #0d6efd; cursor: pointer; font-size: 1.4rem;" data-bs-toggle="tooltip"
                                    title=" CPF: {{ $insc->cpf }} | Grupo: {{ $insc->nomeg }} | Atividade: {{ $insc->trsigla }} - {{ $insc->trnome }} | Observação:{{ $insc->descricao }} | Sala: {{ $insc->numero }} | Hora fim:{{ date('H:i', strtotime($insc->h_fim)) }} | Data fim:{{ $insc->data_fim ? date('d-m-Y', strtotime($reuni->data_fim)) : '-' }}"></i>
                            </td>
                            <td>
                                @if ($insc->statusid == 3)
                                ...
                                @else
                                    <button type="button" class="btn btn-outline-warning btn-sm tooltips" disabled>
                                        <span class="tooltiptext">Ativar</span>
                                        <i class="bi bi-check-circle" style="font-size: 1rem; color:#000;"></i></button>
                                    <a href="/altera-turma/{{ $insc->idi }}"><button type="button"
                                            class="btn btn-outline-warning btn-sm tooltips">
                                            <span class="tooltiptext">Trocar</span>
                                            <i class="bi bi-arrow-left-right"
                                                style="font-size: 1rem; color:#000;"></i></button></a>
                                    <a href="#" class="btn btn-outline-danger btn-sm tooltips"
                                        data-bs-toggle="modal" data-bs-target="#modali{{ $insc->idi }}"
                                        data-tt="tooltip" data-placement="top"><span class="tooltiptext">Inativar</span>
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header" style="background-color:#DC4C64">
                                                    <h5 class="modal-title" id="exampleModalLabel" style="color:white">
                                                        Inativar a inscrição</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body" style="text-align: center; ">
                                                    Tem certeza que deseja inativar a inscrição de<br /><span
                                                        style="color:#DC4C64; font-weight: bold;">{{ $insc->nome_completo }}</span>&#63;
                                                </div>
                                                <center>
                                                    <form action="/inativar-inscricao/{{ $insc->idi }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="mb-2 col-10">
                                                            <label class="col-form-label">Insira o motivo da
                                                                <span style="color:#DC4C64">inativação:</span></label>
                                                            <select class="form-select teste1" name="motivo_inat"
                                                                required>
                                    ...
                                    <a href="#" class="btn btn-outline-danger btn-sm tooltips"
                                        data-bs-toggle="modal" data-bs-target="#modale{{ $insc->idi }}"
                                        data-tt="tooltip" data-placement="top"><span class="tooltiptext">Excluir</span>
                                    ...
                                    <div class="modal fade" id="modale{{ $insc->idi }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header" style="background-color:#DC4C64">
                                                    <h5 class="modal-title" id="exampleModalLabel" style="color:white">
                                                        Excluir a inscrição</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body" style="text-align: center; ">
                                                    Tem certeza que deseja excluir a inscrição de<br /><span
                                                        style="color:#DC4C64; font-weight: bold;">{{ $insc->nome_completo }}</span>&#63;
                                                </div>
                                                <div class="modal-footer mt-3">
                                                    <button type="button" class="btn btn-danger"
                                                    ...
                                                    <a type="button" class="btn btn-primary"
                                                        href="/excluir-inscricao/{{ $insc->idi }}">Confirmar</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    {{-- fim modal exclusão --}}
                                @endif
                                <a href="/visualizar-inscricao/{{ $insc->idi }}"><button type="button"
                                        class="btn btn-outline-info btn-sm tooltips">
        ...
        {{ $inscricao->links('pagination::bootstrap-5') }}
    </div>
