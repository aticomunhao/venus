        ...
        <div class="container-fluid">
            <form method="GET" action="/gerenciar-relatorio-encaminhamento">
                <div class="d-flex flex-wrap align-items-end gap-2 justify-content-start">
                    ...
                    <div class="col-2" style="min-width: 180px;">
                        <label for="tipo_tratamento" class="form-label mb-1">Tipo Tratamento</label>
                        <select name="tipo_tratamento" id="tipo_tratamento" class="form-select select2 tipo_tratamento">
                            <option value="">Todos</option>
                            @foreach ($tipos_tratamento as $tt)
                                <option value="{{ $tt->id }}"
                                    {{ request('tipo_tratamento') == $tt->id ? 'selected' : '' }}>
                                    {{ $tt->descricao }}
                                </option>
                            @endforeach
                        </select>
